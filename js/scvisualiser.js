function AudioPlayer()
{
	...
	*/
	...
	document.getElementById("playButton").addEventListener("click", function() { self.playSong(); }, false);
	soundcloudUrlTextbox.addEventListener("keypress", playOnEnterKey, false);
	player.addEventListener("ended", onMusicEnd, false);
	...
	function onMusicEnd(event)
	{
		document.title = "[Paused] " + self.currentlyPlayingTitle + " - Soundcloud Visualiser";
	}
	
	function getSongPathFromUrl(url)
	{
		return /soundcloud\.com\/(.+)/i.exec(url)[1];
	}
	...
	AudioPlayer.prototype.playSong = function()
	{
		...
		// get the song path from the soundcloud url, does not require protocol
		var songPath = getSongPathFromUrl(soundcloudUrl);
		...
		// now we strictly require the full url including protocol
		soundcloudUrl = "https://soundcloud.com/" + songPath;
		...
		.then(function(resolved) {
			...
			SC.get(uri)
				.then(function(streams) {
					...
					document.title = "[Playing] " + resolved.title + " - Soundcloud Visualiser";
					document.location.hash = encodeURI(getSongPathFromUrl(self.currentlyPlayingUrl));
...
audioPlayer.startRender();
var hash = decodeURI(document.location.hash.substring(1));
if (hash)
{
	soundcloudUrlTextbox.value = "https://soundcloud.com/" + hash;
